...
public interface PaymentsRepository extends JpaRepository<Payment,Long> {
    ...
    @Query("select new com.chargebee.cbgwalerts.model.DomainAndCount(ls.domain ,count(p.site_id))"
            + "from Payment p join LiteSite ls on p.site_id =ls.site_id where p.gateway =:gateway and p.payment_method=:payment and p.status=:status and p.created_at < :localDateTime  group by p.site_id")
